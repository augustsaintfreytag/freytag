# Create Mode: Shut down proxy, let Certbot run in standalone mode for creation, restart proxy.
...
do
	...
	docker run -p 80:80 -v $VOLUME_CHALLENGES:/usr/share/nginx/html/challenges -v $VOLUME_SSL:/etc/letsencrypt certbot/certbot certonly --standalone --preferred-challenges http --agree-tos --email night@apricummedia.com -d $DOMAIN_NAME
done
